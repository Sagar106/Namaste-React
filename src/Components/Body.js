...
const Body = () => {
  ...
  const fetchData = async () => {
    const data = await fetch(`${API_BASE_URL}/listRestaurants`);
      ...
      <div className="flex justify-between items-center">
        <div className="p-3">
          <input
            ...
            type="text"
            className="border-2 px-2 border-amber-500 rounded-md mx-2"
            value={searchText}
            onChange={(e) => setSearchText(e.target.value)}
          />
          <button
            className="px-3 py-1 mx-2 bg-amber-400 rounded-md cursor-pointer hover:bg-amber-50 hover:text-amber-500"
        ...
        <div className="">
          <button
            className="px-5 py-1 mx-5 border-1 text-amber-500 bg-amber-50 rounded-md cursor-pointer hover:bg-amber-400 hover:text-amber-50"
